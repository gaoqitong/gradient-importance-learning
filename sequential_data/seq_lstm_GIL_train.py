...
parser.add_argument("-replay_buffer", type=int, help="Size of experience replay buffer for training actor and critic. Default to 10**5 but can be reduced to 10**4 if training too slow or occupies too much RAM.", default=10**5)
...
if __name__ == '__main__':
    ...
    with graph.as_default():
        ...
        dataset_train = tf.data.Dataset.from_generator(gen_train, (tf.float32, tf.float32, tf.int32), ([ timesteps, df_shock_train.values.shape[1]],[ 2],[timesteps, df_shock_train.values.shape[1]])).repeat(1000).shuffle(5000).batch(batch_size)
    ...
    with tf.Session(config=session_config, graph=graph) as sess:
        ...
        for step in range(training_steps):
            ...
            _, kld = sess.run([grads_update_op, train_kld], feed_dict={grad_importance:a, input_train_holder:data_in, label_train_holder:label_in, mask_train_holder:s_mask})
