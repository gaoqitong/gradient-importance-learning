...
parser.add_argument("-lr_actor", type=float, help="Set learning rate for training the actor DEFAULT=0.00005", default=0.00005)
...
parser.add_argument("-decay_rate", type=float, help="Set exponential decay rate DEFAULT=0.9", default=0.9)
...
if __name__ == '__main__':
    ...
    def gen_train():
        ...
        for i in range(int(data_train.shape[0]/128)):
    ...
    with graph.as_default():
        ...
        dataset_train = tf.data.Dataset.from_generator(gen_train, (tf.float32, tf.float32, tf.int32), ([batch_size,normal_train.shape[1]],[batch_size,2],[batch_size,normal_train.shape[1]])).repeat(10).shuffle(50)
        ...
        grads[0] = grads[0]*importance[...,tf.newaxis]
    ...
    with tf.Session(config=session_config, graph=graph) as sess:
        ...
        for step in range(training_steps):
            ...
            features, _, kld, test_kld = sess.run([feature, grads_update_op, train_kld, final_kld], feed_dict={grad_importance:a, input_train_holder:data_in, label_train_holder:label_in, mask_train_holder:s_mask})
